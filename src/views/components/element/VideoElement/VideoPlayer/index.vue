<template>
  ...
  >
    <div class="video-wrap" @click="toggle()">
      <div class="load-error" v-if="loadError">비디오 로드 실패</div>
      ...
      <div class="icons icons-right">
        <div class="speed">
          <div class="icon speed-icon">
            <span class="icon-content" @click="speedMenuVisible = !speedMenuVisible">{{playbackRate === 1 ? '재생 속도' : (playbackRate + 'x')}}</span>
        ...
        <div class="loop" @click="toggleLoop()">
          <div class="icon loop-icon" :class="{ 'active': loop }">
            <span class="icon-content">반복 {{loop ? '켜짐' : '꺼짐'}}</span>
          </div>
        </div>
      </div>
