<template>
  <div class="table-generator">
    <div class="title">
      <div class="lef">표 {{endCell.length ? `${endCell[0]} x ${endCell[1]}` : ''}}</div>
      <div class="right" @click="isCustom = !isCustom">{{ isCustom ? '돌아가기' : '사용자 정의' }}</div>
    </div>
    <table 
      @mouseleave="endCell = []" 
    ...
    <div class="custom" v-else>
      ...
      <div class="btns">
        ...
        <Button class="btn" type="primary" @click="insertCustomTable()">확인</Button>
      </div>
    </div>
  </div>
</template>
