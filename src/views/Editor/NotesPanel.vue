      ...
      <div class="notes" ref="notesRef">
        <div class="note" :class="{ 'active': activeNoteId === note.id }" v-for="note in notes" :key="note.id" @click="handleClickNote(note)">
          <div class="header note-header">
            ...
            <div class="btns">
              <div class="btn reply" @click="replyNoteId = note.id">답글</div>
              <div class="btn delete" @click.stop="deleteNote(note.id)">삭제</div>
            </div>
          </div>
          ...
          <div class="replies" v-if="note.replies?.length">
            <div class="reply-item" v-for="reply in note.replies" :key="reply.id">
              <div class="header reply-header">
                ...
                <div class="btns">
                  <div class="btn delete" @click.stop="deleteReply(note.id, reply.id)">삭제</div>
                </div>
              </div>
          ...
          <div class="note-reply" v-if="replyNoteId === note.id">
            <TextArea :padding="6" v-model:value="replyContent" placeholder="답글을 입력하세요" :rows="1" @enter.prevent="createNoteReply()" />
            <div class="reply-btns">
              <Button class="btn" size="small" @click="replyNoteId = ''">취소</Button>
              <Button class="btn" size="small" type="primary" @click="createNoteReply()">답글</Button>
            </div>
          </div>
        </div>
        <div class="empty" v-if="!notes.length">이 페이지에는 주석이 없습니다</div>
      </div>
      <div class="send">
        <TextArea 
          ref="textAreaRef"
          ...
          :padding="6"
          :placeholder="`주석 입력 (${handleElementId ? '선택한 요소' : '현재 슬라이드'})`"
          :rows="2"
          @focus="replyNoteId = ''; activeNoteId = ''"
          @enter.prevent="createNote()"
        />
        <div class="footer">
          <IconDelete class="btn icon" v-tooltip="'이 페이지의 주석 모두 삭제'" style="flex: 1" @click="clear()" />
          <Button type="primary" class="btn" style="flex: 12" @click="createNote()">주석 추가</Button>
        </div>
      </div>
    </div>
  </MoveablePanel>
