    ...
    <div class="element-animation" v-if="handleElement">
      ...
      >
        ...
        <Button class="element-animation-btn" @click="handleAnimationId = ''">
          <IconEffects style="margin-right: 5px;" /> 애니메이션 추가
        </Button>
      </Popover>
    </div>

    <div class="tip" v-else><IconClick style="margin-right: 5px;" /> 캔버스에서 요소를 선택하여 애니메이션을 추가하세요</div>
      ...
      <template #item="{ element }">
        <div class="sequence-item" :class="[element.type, { 'active': handleElement?.id === element.elId }]">
          <div class="sequence-content">
            ...
            <div class="handler">
              <IconPlayOne class="handler-btn" v-tooltip="'미리보기'" @click="runAnimation(element.elId, element.effect, element.duration)" />
              <IconCloseSmall class="handler-btn" v-tooltip="'삭제'" @click="deleteAnimation(element.id)" />
            </div>
          </div>
          ...
          <div class="configs" v-if="handleElementAnimation[0]?.elId === element.elId">
            ...
            <div class="config-item">
              <div style="width: 35%;">트리거 방식:</div>
              <Select
                :value="element.trigger"
                @update:value="value => updateElementAnimationTrigger(element.id, value as AnimationTrigger)"
                style="width: 65%;"
                :options="[
                  { label: '클릭 시 실행', value: 'click' },
                  { label: '이전 애니메이션과 동시에', value: 'meantime' },
                  { label: '이전 애니메이션 이후 자동 실행', value: 'auto' },
                ]"
              />
            </div>
            <div class="config-item">
              <Button style="width: 100%;" @click="openAnimationPool(element.id)">애니메이션 변경</Button>
            </div>
          </div>
        </div>
      </template>
...
const tabs: TabItem[] = [
  { key: 'in', label: '시작', color: '#68a490' },
  { key: 'out', label: '끝', color: '#d86344' },
  { key: 'attention', label: '강조', color: '#e8b76a' },
]
