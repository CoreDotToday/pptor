<template>
  <div class="theme-colors-setting">
    <div class="title">차트 테마 색상</div>

    <div class="list">
      <div class="row" v-for="(item, index) in themeColors" :key="index">
        <div class="label" style="width: 40%;">테마 색상 {{ index + 1 }}:</div>
        <Popover trigger="click" style="width: 60%;">
          ...
          <div class="color-btn-wrap" style="width: 100%;">
            ...
            <div class="delete-color-btn" v-tooltip="'삭제'" @click.stop="deleteThemeColor(index)" v-if="index !== 0"><IconCloseSmall /></div>
          </div>
        </Popover>
      </div>
      <Button
        ...
        @click="addThemeColor()"
      >
        <IconPlus class="btn-icon" /> 테마 색상 추가
      </Button>
    </div>

    <Button class="btn" type="primary" @click="setThemeColors()">확인</Button>
  </div>
</template>
