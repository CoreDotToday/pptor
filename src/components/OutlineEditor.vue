...
const deleteItem = (itemId: string, isTitle?: boolean) => {
  if (isTitle) {
    ...
    if (nextItem && nextItem.lv > item.lv) {
      message.error('먼저 하위 항목을 모두 삭제하세요')
      return
    }
  }
...
const contextmenus = (el: HTMLElement): ContextmenuItem[] => {
  ...
  if (lv === 1) {
    ...
    return [
      {
        text: '위에 동등한 개요 추가 (장)',
        handler: () => addItem(id, 'prev', '새 장'),
      },
      {
        text: '하위 개요 추가 (절)',
        handler: () => addItem(id, 'next', '새 절'),
      },
      {
        text: '이 장 삭제',
    ...
    return [
      {
        text: '위에 동등한 개요 추가 (절)',
        handler: () => addItem(id, 'prev', '새 절'),
      },
      {
        text: '하위 개요 추가 (항목)',
        handler: () => addItem(id, 'next', '새 항목'),
      },
      {
        text: '이 절 삭제',
        handler: () => deleteItem(id, true),
  ...
  return [
    {
      text: '위에 동등한 개요 추가 (항목)',
      handler: () => addItem(id, 'prev', '새 항목'),
    },
    {
      text: '아래에 동등한 개요 추가 (항목)',
      handler: () => addItem(id, 'next', '새 항목'),
    },
    {
      text: '이 항목 삭제',
